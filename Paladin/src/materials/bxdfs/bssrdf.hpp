...
public:
	...
	Spectrum S(const SurfaceInteraction &pi, const Vector3f &wi) {
		...
		Float Ft = FrDielectric(Frame::cosTheta(_po.wo), 1, _eta);
	...
	Spectrum Sw(const Vector3f &w) const {
		...
		return (1 - FrDielectric(Frame::cosTheta(w), 1, _eta)) / (c * Pi);
	}
 ...
 * 函数值小的地方，样本间隔可以大一些
 * 
 * 接下来介绍鄙人认为最难的部分，如何生成Sr函数的离散表结构
 * 
 * 在将一般的传递方程转化为扩散方程的过程中，采用了许多重要的思想，
 ...
 *
 * 这里记录一下个人理解，如果强制把各项异性相函数换成各项同性，那么各项异性的特性需要包含在
 * 表达式中，如何包含呢？方法就是在以上三个系数中加入各向异性数据，
 * 就是经过修正的基本系数，包含了各向异性参数，至于为何是这个表达式，应该是科学家经过反复试验
 * 计算出的最优形式
 *
 * 
 *
 ...
 * 5式联合volpath.hpp中14式
 *
 *     Ls(p, ω) = Le(p, ω) + σs(p, ω) ∫[sphere]p(p, ωi, ω) Li(p, ωi) dωi    6式
 *
 * 可得
 *
 *    dLo(p + tω, ω)/dt = −σt(p, ω) Li(p, −ω) 
 *              + Le(p, ω) + σs(p, ω) ∫[sphere]p(p, ωi, ω) Li(p, ωi) dωi     7式
 *
 * 我们假设空间上均匀的材质，并转换到各项同性相函数p = 1/4π
 * 利用相似理论对散射衰减系数作相应的改变。同时也将
 * Lo(p, ω) = Li(p, -ω)替换为L(p, ω)   (这点不理解，为何可以这样替换，难道是因为各向同性的缘故？)
 *
 *    dL(p + tω, ω)/dt = −σ't L(p, ω) 
 *
 *                             σ's 
 *              + Le(p, ω) +  ----- ∫[sphere] Li(p, ωi) dωi     8式
 *                             4π
 * 
 * 再强调一下，7式的形式转到8式的形式的基本思路就是，把材质的各项异性系数转移到了
 * 其他参数中，这样做的好处是可以给Sr函数降维，减少储存所需要的空间
 * 
 * 扩散理论(diffusion theory)的关键假设是，每一次散射事件都会模糊入射光强，
 * 当光进一步传播到介质中时，高频率从角辐射分布中消失。这还是比较容易理解的，
 * 用激光笔直射皮肤的那种效果就可以看出来了
 * 
 * 在稠密的各向同性散射介质中，最终会失去所有的方向性。在这个观测的激励下，
 * 辐射函数被限制为一个简单的基于球面矩的两项展开。
 * 
 * 但单位球上的n阶矩
 * 形式上为  (μn[f])i,j,k... = ∫[sphere] ωi ωj ωk ... f(ω)dω
 *                                      -------------
 *                                         n个系数
 * 
 * 换句话说，我们需要求解在笛卡尔坐标系中，f(ω)乘以各个方向分量的积分
