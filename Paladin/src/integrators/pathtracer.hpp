 ...
 *                n=1
 *     _
 * 其中pn = p0,p1,p2......pn，表示由n+1个点构成的长度为n的路径
 *
 *   _                         n-1
 * P(pn) = ∫∫...∫ Le(pn→pn-1) ( ∏ f(pi+1 → pi → pi-1) G(pi+1 ↔ pi) ) dA(p2)...dA(pn)
 *          n - 1              i=1
 *
 * 把中间这部分抽出来，形式如下，我们把它称为吞吐量 (throughout)
 *
 *    _     n-1
 *  T(pn) =  ∏ f(pi+1 → pi → pi-1) G(pi+1 ↔ pi) ) dA(p2)...dA(pn)
 *          i=1
 * 
 * 
 * 
