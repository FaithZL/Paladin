...
Spectrum PathTracer::Li(const RayDifferential &r, const Scene &scene,
    ...
    for (bounces = 0; ; ++bounces) {
        ...
        bsdfVal *= absDot(isect.shading.normal, wi);
        ...
        bool foundSurfaceInteraction = scene.rayIntersect(ray, &lightIsect);
        if (!foundSurfaceInteraction) {
            break;
        }
        const Light * light2 = lightIsect.shape->getAreaLight();
        Spectrum Li(0.0f);
        if (light2 == light) {
            Li = lightIsect.Le(-wi);
        }
        Float lightPdf = light2 ? light2->pdf_Li(isect, wi) : 0;
